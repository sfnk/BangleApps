...
// Actually draw the watch face
  let draw = function() {
    ...
    g.setFontAlign(0, 0).setFont("4x6", 2).drawString(uiLogText, x, y+48);
    // queue next draw
  ...
  */
  ...
  let gattfunc = function(addr){
    NRF.requestDevice({ filters: [{ id: addr }] }).then(function(device) {
      uiLog("found device");
      ...
      uiLog("bonded", gatt.getSecurityStatus());
      gatt.disconnect();
    }).catch(function(e) {
      uiLog(e.toString());
    });
  ...
  NRF.on('advertising', function(isAdvertising) {
    if(isAdvertising){
      ...
      uiLog("advertising off");
    }
  });

  NRF.on('servicesDiscover', function() {
    uiLog('servicesDiscover');
  });
}